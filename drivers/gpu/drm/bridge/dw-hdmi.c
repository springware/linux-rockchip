...
static void hdmi_mask_writel(struct dw_hdmi *hdmi, u32 data, unsigned int reg,
			     u32 shift, u32 mask)
{
	hdmi->mod(hdmi, data << shift, mask, reg);
}
...
static const struct rk32_hdmi_reg_table hdmi_reg_table[] = {
	...
	{HDMI_VP_STATUS, HDMI_VP_MASK},
	{HDMI_FC_INVIDCONF, HDMI_FC_DBGTMDS2},
	{HDMI_PHY_CONF0, HDMI_PHY_PLLCFGFREQ2},
	{HDMI_PHY_I2CM_SLAVE_ADDR, HDMI_PHY_I2CM_SDA_HOLD},
	{HDMI_AUD_CONF0, HDMI_AHB_DMA_STPADDR_SET1_0},
	{HDMI_MC_SFRDIV, HDMI_MC_SWRSTZREQ_2},
	{HDMI_CSC_CFG, HDMI_CSC_SPARE_2},
	{HDMI_A_HDCPCFG0, HDMI_HDCP_REVOC_LIST},
	{HDMI_HDCPREG_BKSV0, HDMI_HDCPREG_BKSV4},
	{HDMI_HDCPREG_ANCONF, HDMI_HDCPREG_AN7},
	{HDMI_HDCPREG_RMCTL, HDMI_HDCPREG_DPK6},
...
static int __maybe_unused hdmi_phy_i2c_read(struct dw_hdmi *hdmi, int addr)
{
	int val = 0;

	hdmi->write(hdmi, addr, HDMI_PHY_I2CM_ADDRESS_ADDR);
	hdmi->write(hdmi, 0x00, HDMI_PHY_I2CM_DATAO_1_ADDR);
	hdmi->write(hdmi, 0x00, HDMI_PHY_I2CM_DATAO_0_ADDR);
	hdmi->write(hdmi, HDMI_PHY_I2CM_OPERATION_ADDR_READ,
		    HDMI_PHY_I2CM_OPERATION_ADDR);

	if (hdmi_phy_wait_i2c_done(hdmi, 1000) == false) {
		dev_err(hdmi->dev, "hdmi phy write failed\n");
	} else {
		val = hdmi->read(hdmi, HDMI_PHY_I2CM_DATAI_1_ADDR);
		val = (val & 0xff) << 8;
		val += (hdmi->read(hdmi, HDMI_PHY_I2CM_DATAI_0_ADDR) & 0xff);
	}
...
#if defined(CONFIG_DEBUG_FS)
static int rockchip_hdmi_reg_show(struct seq_file *s, void *v)
{
	int i = 0, j = 0;
	u32 val = 0;
	...
	for (i = 0; i <= 0x26; i++) {
		...
		if (i && (i%6 == 0))
			seq_puts(s, "\n");
		seq_printf(s, " %04x", val);
	}

	return 0;
}

static ssize_t rockchip_hdmi_reg_write(struct file *file,
				       ...
				       size_t count, loff_t *ppos)
{
	...
	if ((reg < 0) || (reg > HDMI_I2CM_SCDC_UPDATE1)) {
		dev_dbg(hdmi->dev, "it is no hdmi reg\n");
		return count;
	}
	DRM_DEBUG_DRIVER("/********** rockchip hdmi reg config ******/");
	DRM_DEBUG_DRIVER("\n reg=%x val=%x\n", reg, val);
...
static int rockchip_hdmi_reg_open(struct inode *inode, struct file *file)
{
	struct dw_hdmi *hdmi = inode->i_private;

	return single_open(file, rockchip_hdmi_reg_show, hdmi);
}

static const struct file_operations dw_hdmi_reg_fops = {
	.owner = THIS_MODULE,
	.open = rockchip_hdmi_reg_open,
	.read = seq_read,
	.write = rockchip_hdmi_reg_write,
	.llseek = seq_lseek,
...
int dw_hdmi_connector_mode_valid(struct drm_connector *connector,
				  struct drm_display_mode *mode)
{
	return MODE_OK;
}
...
static int dw_hdmi_register(struct drm_device *drm, struct dw_hdmi *hdmi)
{
	...
	if (hdmi->dev_type == IMX6DL_HDMI || hdmi->dev_type == IMX6Q_HDMI) {

#if defined(CONFIG_DRM_IMX)
		if (imx_drm_encoder_parse_of(drm, encoder, dev->of_node));
			return -EINVAL;
#endif
	} else {
...
static int dw_hdmi_bind(struct device *dev, struct device *master, void *data)
{
	...
	debugfs_create_file("hdmi", S_IRUSR, hdmi->debugfs_dir,
